...
def test_oncall_service__get_current_shift_returns_none_if_shift_not_exists(now: dt.datetime, rotation: Rotation):
    svc = OncallService(InMemoryStoreFactory())
    svc.create_rotation(rotation)
...
@pytest.mark.parametrize("now", [dt.datetime(2025, 1, 1), dt.datetime(2025, 1, 2)], ids=["start-shift", "mid-shift"])
def test_oncall_service__get_shifts(now: dt.datetime, rotation: Rotation):
    ...
    assert actual_shifts == [
        Shift(id=shifts[1].id, firefighter='f2',
              start_date=dt.datetime(2025, 1, 1, 0, 0), end_date=dt.datetime(2025, 1, 3, 0, 0)),
        Shift(id=shifts[2].id, firefighter='f3',
              start_date=dt.datetime(2025, 1, 3, 0, 0), end_date=dt.datetime(2025, 1, 7, 0, 0)),
        Shift(id=shifts[3].id, firefighter='f1',
              start_date=dt.datetime(2025, 1, 7, 0, 0), end_date=dt.datetime(2025, 1, 9, 0, 0))
    ]
...
@pytest.mark.parametrize("now", [dt.datetime(1990, 1, 1), dt.datetime(2026, 1, 1)], ids=["past", "future"])
def test_oncall_service__get_shifts_returns_empty_list_if_shift_not_exists(now: dt.datetime, rotation: Rotation):
    svc = OncallService(InMemoryStoreFactory())
    svc.create_rotation(rotation)
